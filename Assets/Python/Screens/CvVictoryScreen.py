...
class CvVictoryScreen:
	...
	def showGameSettingsScreen(self):
		...
		#screen.appendListBoxStringNoUpdate(szSettingsTable, u"     (" + CyGameTextMgr().parseLeaderTraits(activePlayer.getLeaderType(), activePlayer.getCivilizationType(), True, False) + ")", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY ) # edead
		...
		screen.appendListBoxStringNoUpdate(szSettingsTable, " ", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
		#screen.appendListBoxStringNoUpdate(szSettingsTable, localText.getText("TXT_KEY_SETTINGS_STARTING_ERA", (gc.getEraInfo(gc.getGame().getStartEra()).getTextKey(), )), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY ) # edead
		...
		for iLoopPlayer in range(gc.getMAX_CIV_PLAYERS()):
			...
			if (player.isEverAlive() and iLoopPlayer != self.iActivePlayer and (gc.getTeam(player.getTeam()).isHasMet(activePlayer.getTeam()) or gc.getGame().isDebugMode()) and not player.isBarbarian() and not player.isMinorCiv()):
				...
				#screen.appendListBoxStringNoUpdate(szCivsTable, u"     (" + CyGameTextMgr().parseLeaderTraits(player.getLeaderType(), player.getCivilizationType(), True, False) + ")", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY ) # edead
				#screen.appendListBoxStringNoUpdate(szCivsTable, " ", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY ) # edead
	...
	def showVictoryConditionScreen(self):
		...
		# edead: start
		# iOurReligion = gc.getPlayer(self.iActivePlayer).getStateReligion()
		# ourReligionPercent = 0
		# if iOurReligion != -1: 
			# ourReligionPercent = gc.getGame().calculateReligionPercent(iOurReligion)
		# edead: end
		...
		for iLoopVC in range(gc.getNumVictoryInfos()):
			...
			if gc.getGame().isVictoryValid(iLoopVC):
				...
				# Unique Religious Victory moved further
				# if (victory.getReligionPercent() > 0):			
					# iRow = screen.appendTableRow(szTable)
					# edead: start
					# religionPercent = victory.getReligionPercent()
					# if iOurReligion == gc.getInfoTypeForString("RELIGION_SUNNI"):
						# religionPercent += 10
					# elif iOurReligion == gc.getInfoTypeForString("RELIGION_ORTHODOXY"):
						# religionPercent += 5
					# elif iOurReligion == gc.getInfoTypeForString("RELIGION_CATHOLICISM"):
						# religionPercent -= 5
				...
				# edead: start Unique Religious Victory
				if iLoopVC == 7:
					if not gc.getPlayer(self.iActivePlayer).isMinorCiv() and not gc.getPlayer(self.iActivePlayer).isBarbarian():
						iReligion = gc.getPlayer(self.iActivePlayer).getStateReligion()
						if iReligion != -1:
							for i in range(3):
								iRow = screen.appendTableRow(szTable)
								screen.setTableText(szTable, 0, iRow, localText.getText(con.tReligiousGoals[iReligion][i], ()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
								...
								if sd.getReligiousGoal(self.iActivePlayer, i) == 1:
								...
								elif sd.getReligiousGoal(self.iActivePlayer, i) == 0:
								...
								aHelpStrings = vic.getURV(self.iActivePlayer, i, bHelp=True)
								if aHelpStrings:
							...
							bEntriesFound = True
				# edead: end Unique Religious Victory
					
				#Rhye - start
				if iLoopVC == 8:
					if not gc.getPlayer(self.iActivePlayer).isMinorCiv() and not gc.getPlayer(self.iActivePlayer).isBarbarian(): # edead
						for i in range(3):
							iRow = screen.appendTableRow(szTable)
							...
							screen.setTableText(szTable, 2, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_ACCOMPLISHED", ()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
							if sd.getGoal(self.iActivePlayer, i) == 1:
								screen.setTableText(szTable, 3, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_YES", ()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
							elif sd.getGoal(self.iActivePlayer, i) == 0:
								screen.setTableText(szTable, 3, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_NO", ()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
							else:
								screen.setTableText(szTable, 3, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_NOTYET", ()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
