...
def getUniquePowerName(argsList):
	"""Returns the name key for eCivilization's UP."""
	eCivilization = argsList[0]
	if -1 < eCivilization < len(con.tUniquePowers):
		return con.tUniquePowers[eCivilization][0]
	return "TXT_KEY_UNIQUE_POWER_NONE"

def getUniquePowerDescription(argsList):
	"""Returns the description text key for eCivilization's UP."""
	eCivilization = argsList[0]
	if -1 < eCivilization < len(con.tUniquePowers):
		return con.tUniquePowers[eCivilization][1]
	return " "

def getGoalText1(argsList):
	"""Returns the text key for eCivilization's UHV1 for the last game speed i.e. Normal."""
	eCivilization = argsList[0]
	if -1 < eCivilization < len(con.tGoals[-1:][0]):
		return con.tGoals[-1:][0][eCivilization][0]
	return "None"

def getGoalText2(argsList):
	"""Returns the text key for eCivilization's UHV2 for the last game speed i.e. Normal."""
	eCivilization = argsList[0]
	if -1 < eCivilization < len(con.tGoals[-1:][0]):
		return con.tGoals[-1:][0][eCivilization][1]
	return "None"

def getGoalText3(argsList):
	"""Returns the text key for eCivilization's UHV3 for the last game speed i.e. Normal."""
	eCivilization = argsList[0]
	if -1 < eCivilization < len(con.tGoals[-1:][0]):
		return con.tGoals[-1:][0][eCivilization][2]
	return "None"

def getStartingYear(argsList):
	eCivilization = argsList[0]
	if -1 < eCivilization < len(con.tBirth):
		return con.tBirth[eCivilization]
	return -1

def getLoadingTime(argsList):
	eCivilization = argsList[0]
	if -1 < eCivilization < len(con.tLoadingTime):
		return con.tLoadingTime[eCivilization]
	return -1

def getRating1(argsList):
	eCivilization = argsList[0]
	if -1 < eCivilization < len(con.tRating):
		return con.tRating[eCivilization][0]
	return 0

def getRating2(argsList):
	eCivilization = argsList[0]
	if -1 < eCivilization < len(con.tRating):
		return con.tRating[eCivilization][1]
	return 0

def getRating3(argsList):
	eCivilization = argsList[0]
	if -1 < eCivilization < len(con.tRating):
		return con.tRating[eCivilization][2]
	return 0

def getRating4(argsList):
	eCivilization = argsList[0]
	if -1 < eCivilization < len(con.tRating):
		return con.tRating[eCivilization][3]
	return 

def getRating5(argsList):
	eCivilization = argsList[0]
	if -1 < eCivilization < len(con.tRating):
		return con.tRating[eCivilization][4]
	return 0
