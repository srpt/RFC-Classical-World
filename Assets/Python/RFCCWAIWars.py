...
#####################################  
	...
	def aiSlaveFunction(self, iPlayer, tPlot):
	
		pPlayer = gc.getPlayer(iPlayer)
		
		# if building a wonder, rush it
		apCityList = PyPlayer(iPlayer).getCityList()
		for pCity in apCityList:
			city = pCity.GetCy()
			if con.iGreatWall <= city.getProductionBuilding() <= con.iShwedagonPaya and city.productionLeft() > 45:
				city.changeProduction(60)
				#CyInterface().addMessage(utils.getHumanID(), False, con.iDuration, CyTranslator().getText("wonder rushed", ()), "", 0, "", ColorTypes(con.iRed), -1, -1, True, True)
				return
			
		# else see if a city needs a settled slave
		iFoodDifference = 5
		for pCity in apCityList:
			city = pCity.GetCy()
			if city.foodDifference(False) > iFoodDifference:
				targetCity = city
				iFoodDifference = city.foodDifference(False)
		if iFoodDifference > 5:
			targetCity.changeFreeSpecialistCount(con.iSettledSlave, 1)
			#CyInterface().addMessage(utils.getHumanID(), False, con.iDuration, CyTranslator().getText("slave settled", ()), "", 0, "", ColorTypes(con.iRed), -1, -1, True, True)
			return
				
		# else if in financial trouble, sell the slave
		if pPlayer.AI_isFinancialTrouble():
			pPlayer.changeGold(30)
			#CyInterface().addMessage(utils.getHumanID(), False, con.iDuration, CyTranslator().getText("slave sold, financial trouble", ()), "", 0, "", ColorTypes(con.iRed), -1, -1, True, True)
			return
				
		# else see if anything else can be rushed
		for pCity in apCityList:
			city = pCity.GetCy()
			if city.getProductionBuilding() >= 0 and city.productionLeft() > 45:
				city.changeProduction(60)
				#CyInterface().addMessage(utils.getHumanID(), False, con.iDuration, CyTranslator().getText("building rushed", ()), "", 0, "", ColorTypes(con.iRed), -1, -1, True, True)
				return
			
		
			
		# else see if workers are needed
		for pCity in apCityList:
			city = pCity.GetCy()
			if pPlayer.AI_unitValue(gc.getCivilizationInfo(pPlayer.getCivilizationType()).getCivilizationUnits(con.iWorker), UnitAITypes.UNITAI_WORKER, pCity.GetCy().area()) > 0:
				pNewUnit = utils.makeUnit(con.iSlave, iPlayer, (city.getX(),city.getY()),1)
				pNewUnit.finishMoves()
				#CyInterface().addMessage(utils.getHumanID(), False, con.iDuration, CyTranslator().getText("slave spawned", ()), "", 0, "", ColorTypes(con.iRed), -1, -1, True, True)
				return
				
		# else sell the slave
		pPlayer.changeGold(30)
		#CyInterface().addMessage(utils.getHumanID(), False, con.iDuration, CyTranslator().getText("slave sold, no needs", ()), "", 0, "", ColorTypes(con.iRed), -1, -1, True, True)
		return
	...
	def onCityRazed(self, argsList):
		...
		if gc.getPlayer(iPlayer).getCivics(2) == con.iSlaveryCivic:
			if iPlayer != utils.getHumanID() and gc.getPlayer(iPlayer).getCapitalCity().isProductionBuilding():
				...
				for i in range(iPopulation):
					pNewUnit = utils.makeUnit(con.iSlave, iPlayer, (city.getX(), city.getY()), 1)
					#pNewUnit = gc.getPlayer(iPlayer).initUnit(con.iSlave, city.getX(), city.getY())
					pNewUnit.finishMoves()
...
#############
	...
	def sungaRevolution(self):
		...
		utils.setCivDesc(con.iMauryans, "Sunga Kingdom", "Sunga", "Sungas", "Pusyamitra")
		CyInterface().addMessage(utils.getHumanID(), True, con.iDuration, CyTranslator().getText("TXT_KEY_INDEPENDENCE_TEXT_SUNGAS", ()), "", 0, "", ColorTypes(con.iRed), -1, -1, True, True)
