...
##			g_bDisplayMercenaryMessages = config.getboolean("Mercenaries Mod", "Display Mercenary Messages", true)
	...
	def onBeginGameTurn(self, argsList):
		...
		# edead: start creating mercenaries 30 turns prior to human spawn
		if gc.getGame().getGameTurn() < getTurnForYear(con.tBirth[gc.getGame().getActivePlayer()]) - 30:
			return
		...
		if(g_bGameTurnMercenaryCreation):
			...
			if (iGameTurn % iUpdatePeriod == (iUpdatePeriod-1)):
				...
				szBuffer = objMercenaryUtils.addMercenariesToPool()
				if szBuffer and gc.getDefineINT("MERCENARY_POOL_ALERT_ENABLE") and gc.getPlayer(gc.getGame().getActivePlayer()).getNumCities() > 0:
					CyInterface().addMessage(gc.getGame().getActivePlayer(), True, con.iDuration, CyTranslator().getText("TXT_KEY_MERCENARY_POOL_ALERT", (szBuffer, )), "", InterfaceMessageTypes.MESSAGE_TYPE_INFO, "", ColorTypes(con.iWhite), -1, -1, False, False)
				# edead: end
			#Rhye - end     
				...
				for mercenaryName, mercenary in playerMercenaries.iteritems():
					if mercenary["iPlacementTurn"] == -1:
						...
						if unit.getID() == -1:
							...
							sd.setMercenaryData("HiredMercenaries", mercenaries)
		# edead: end
	...
	def onUnitLost(self, argsList):
		...
		# edead: commented out - mercenaries cannot upgrade
	...
	def onUnitPromoted(self, argsList):
		...
		# edead: no contracting out so no need to update this for the AI
