...
#import MercenaryNameUtils # edead: replaced by local getRandomMercenaryName
...
from SorenRand import shuffle # edead
...
# their promotions.
# Default value is 10
g_iMercenaryPromotionChance = 4 #Rhye
...
#                           - Civilization Edge
#                           - Random
#							- Province (by SoI provinces, added by edead)
...
g_strMercenaryStartingLocation = "Province" # edead
...
g_iMaxGlobalMercenaryPoolSize = 20 #Rhye
...
##			# Debug code - end
	...
	def getRandomMercenary(self):
		...
		while(len(promotionList) < g_iMinimumStartingMercenaryPromotionCount):
			...
			iMercenaryIndex = gc.getGame().getSorenRandNum(len(con.tMercenaries), "Random mercenary unit") # edead
			iUnitType = con.tMercenaries[iMercenaryIndex][0] # edead
			nameList = con.tMercenaries[iMercenaryIndex][1] # edead
			...
			mercenary = Mercenary(self.getRandomMercenaryName(iUnitType, nameList), gc.getUnitInfo(iUnitType), promotionList, tmpUnit.getExperience(), tmpUnit.experienceNeeded(), iMercenaryIndex)
			# edead - end
	...
	def getEraAppropriateRandomMercenary(self, iEra):
		...
		# and I could have used the time instead to change the variable name but
		# I did not ... so live with it.
		barbarianPlayer = gc.getPlayer(con.iBarbarian) # edead
...
##			        #Rhye - end
	...
	# edead: start new method for generated names
	def getRandomMercenaryName(self, iUnitType, nameList):
		...
		shuffle(randList)
		bFound = False
		for firstName in randList:
			mercenaryName = localText.getText(firstName,()) + " " + lastName
			if not mercenaryName in mercenaries:
				bFound = True
				break
		
		if not bFound:
			while mercenaryName in mercenaries:
				mercenaryName += " "
	...
	def getEraAppropriateUnitPromotionList(self, objUnit, iEra):
		...
		for i in range(gc.getNumPromotionInfos()):
			...
			# edead: Check tech prereq
			if promotionInfo.getTechPrereq() >= 0 and gc.getTechInfo(promotionInfo.getTechPrereq()).getEra() > iEra:
				continue
			
			# edead: Check the unit's blacklist
			if objUnit.getUnitType() in con.mercenaryPromotionBlacklist:
				if i in con.mercenaryPromotionBlacklist[objUnit.getUnitType()]:
					continue
			
			# edead: Check the odds
			if i >= len(con.mercenaryPromotionOdds) or gc.getGame().getSorenRandNum(100, 'Random promotion') > con.mercenaryPromotionOdds[i]:
				continue
	...
	def hireMercenary(self, mercenaryName, iPlayer):
		...
		if(not g_bDelayMercenaryPlacement):
			...
			if mercenary.getUnitInfo().getDomainType() == gc.getInfoTypeForString("DOMAIN_LAND"):
				city = self.getMercenaryStartingLocation(iPlayer, mercenary)
			else:
				city = self.getCoastalStartingLocation(iPlayer, mercenary) 
			# edead: end
	...
	def placeMercenaries(self, iPlayer):
		...
		for mercenary in mercenariesToPlace:
			...
			if tmpMercenary.getUnitInfo().getDomainType() == gc.getInfoTypeForString("DOMAIN_LAND"):
				objCity = self.getMercenaryStartingLocation(iPlayer, tmpMercenary)
			else:
				objCity = self.getCoastalStartingLocation(iPlayer, tmpMercenary)
	...
	def addMercenariesToPool(self):
		...
		# edead: Make a list of names of cities where mercenaries can be recruited
		cityNames = []
		...
		for i in range(maxMercenaryCount):
			...
			if objMercenary:
				mercenaryNames[objMercenary.getName()] = objMercenary.getName()
				if objMercenary.canHireUnit(gc.getGame().getActivePlayer()):
					cityName = self.getMercenaryStartingCityName(gc.getGame().getActivePlayer(), objMercenary)
					if cityName and not cityName in cityNames:
		...
		# edead: Return the city name list as a formatted string
		output = ""
		for i in range(len(cityNames)):
			output += cityNames[i]
			if len(cityNames) > 1:
				if i == len(cityNames) - 2:
					output += " and "
				elif i != len(cityNames) - 1:
					output += ", "
	...
	def getCoastalStartingLocation(self, iPlayer, mercenary=None):
		"""Returns a coastal city that is the closest to the capital."""
		
		# If a list of regions is provided, use the region-based method instead
		if mercenary:
			if len(con.tMercenaries[mercenary.iMercenaryIndex]) > 4:
				return self.getCityByRegion(iPlayer, con.tMercenaries[mercenary.iMercenaryIndex][3], bCoastal=True, lCorporations=con.tMercenaries[mercenary.iMercenaryIndex][4], iSeed=self.getSeedFromStr(mercenary.getName()))
			else:
				return self.getCityByRegion(iPlayer, con.tMercenaries[mercenary.iMercenaryIndex][3], bCoastal=True, iSeed=self.getSeedFromStr(mercenary.getName()))
	...
	def getMercenaryStartingLocation(self, iPlayer, mercenary=None):
		...
		# edead: start
		elif (g_strMercenaryStartingLocation == "Province"):
			if mercenary:
				if len(con.tMercenaries[mercenary.iMercenaryIndex]) > 4:
					objCity = self.getCityByRegion(iPlayer, con.tMercenaries[mercenary.iMercenaryIndex][3], lCorporations=con.tMercenaries[mercenary.iMercenaryIndex][4], iSeed=self.getSeedFromStr(mercenary.getName()))
				else:
					objCity = self.getCityByRegion(iPlayer, con.tMercenaries[mercenary.iMercenaryIndex][3], iSeed=self.getSeedFromStr(mercenary.getName()))
			if not objCity:
				objCity = player.getCapitalCity()
		# edead: end
	...
	# edead: start
	def getSeedFromStr(self, s):
		result = 0
		for c in s:
			result += ord(c)
		return result
	
	def getCityByRegion(self, iPlayer, regionList, bCoastal=False, lCorporations=[], iSeed=0):
		'Returns the best city in a given list of provinces.'
		possibleCityList = []
		apCityList = PyPlayer(iPlayer).getCityList()
		for pCity in apCityList:
			city = pCity.GetCy()
			if lCorporations:
				...
				if not bValid:
					continue
			if utils.getPlotRegion(city.plot()) in regionList:
				if not bCoastal or (bCoastal and city.isCoastal(gc.getMIN_WATER_SIZE_FOR_OCEAN())):
					possibleCityList.append(city)
		if possibleCityList:
			return possibleCityList[iSeed % len(possibleCityList)]
	
	def getMercenaryStartingCityName(self, iPlayer, mercenary):
		'Returns the name of the starting city of a given mercenary type.'
		if 0 > mercenary.iMercenaryIndex > len(con.tMercenaries):
			return "????"
		bCoastal = False
		if gc.getUnitInfo(mercenary.getUnitInfoID()).getDomainType() == 0:
			bCoastal = True
		# make a seed from the character name - this will determine his location
		if len(con.tMercenaries[mercenary.iMercenaryIndex]) > 4:
			city = self.getCityByRegion(iPlayer, con.tMercenaries[mercenary.iMercenaryIndex][3], bCoastal, lCorporations=con.tMercenaries[mercenary.iMercenaryIndex][4], iSeed=self.getSeedFromStr(mercenary.getName()))
		else:
			city = self.getCityByRegion(iPlayer, con.tMercenaries[mercenary.iMercenaryIndex][3], bCoastal, iSeed=self.getSeedFromStr(mercenary.getName()))
		if city:
			return city.getName()
		elif gc.getPlayer(iPlayer).getNumCities() > 0:
	...
	def canContractOutUnit(self, objUnitInfo, bNavalUnits = true):
		...
		# edead: commented out since with the whitelist this is redundant
...
class Mercenary:
	...
	# edead: List index of mercenary type in Consts.py
	iMercenaryIndex = -1
	
			
	#def __init__(self, mercenaryName, objUnitInfo, promotionList, iExperienceLevel, iNextExperienceLevel):
	def __init__(self, mercenaryName, objUnitInfo, promotionList, iExperienceLevel, iNextExperienceLevel, iMercenaryIndex=-1): # edead
		...
		self.iMercenaryIndex = iMercenaryIndex # edead
	...
	def loadData(self, objDict):
		...
		self.iMercenaryIndex = objDict["iMercenaryIndex"] # edead
	...
	def getDictionaryRepresentation(self):
		...
		objDict["iMercenaryIndex"] = self.iMercenaryIndex # edead
	...
	def place(self, objCity):
		...
		if(g_bDisplayMercenaryMessages):
			strMessage = localText.getText("TXT_KEY_MERCENARY_ARRIVED_AT", (rstrip(self.getName()), objCity.getName(),))
	...
	def hire(self, iPlayer, objCity):
		...
		if(g_bDisplayMercenaryMessages and self.iBuilder != -1):
			strMessage = localText.getText("TXT_KEY_MERCENARY_HIRED_BY", (rstrip(self.getName()), gc.getPlayer(self.iOwner).getName(),))
	...
	def fire(self):
		...
		if(g_bDisplayMercenaryMessages and self.iBuilder != -1 and self.iBuilder != self.iOwner):
			strMessage = localText.getText("TXT_KEY_MERCENARY_FIRED_BY", (rstrip(self.getName()), gc.getPlayer(self.iOwner).getName(),))
	...
	def canHireUnit(self, iPlayer, bNavalUnits = True):
		...
		# Check for companies if applicable.
		regionList = []
		bHasPorts = False
		apCityList = PyPlayer(iPlayer).getCityList()
		for pCity in apCityList:
			city = pCity.GetCy()
			bValid = True
			if len(con.tMercenaries[self.iMercenaryIndex]) > 4:
				bValid = False
				for iCorporation in con.tMercenaries[self.iMercenaryIndex][4]:
					if city.isHasCorporation(iCorporation):
						bValid = True
			if bValid:
				regionList.append(utils.getPlotRegion(city.plot()))
			if bNavalUnits and not bHasPorts:
				if city.isCoastal(gc.getMIN_WATER_SIZE_FOR_OCEAN()):
					bHasPorts = True
		
		# naval units
		if gc.getUnitInfo(unitType).getDomainType() == gc.getInfoTypeForString("DOMAIN_SEA") and bNavalUnits and not bHasPorts:
			return False
			
		# all units: state religion + regions
		if not con.tMercenaries[self.iMercenaryIndex][2] or iStateReligion in con.tMercenaries[self.iMercenaryIndex][2]:
			return set(regionList) & con.tMercenaries[self.iMercenaryIndex][3]
