...
def getScoreComponent(iRawScore, iInitial, iMax, iFactor, bExponential, bFinal, bVictory, iCiv=-1): # edead
	...
	if bVictory:
		...
		# edead: bonus for victory conditions that require longer gameplay, to offset tVictory penalty
		if gc.getGame().getVictory() in [gc.getInfoTypeForString("VICTORY_DOMINATION"), gc.getInfoTypeForString("VICTORY_CONQUEST"), gc.getInfoTypeForString("VICTORY_RELIGIOUS"), gc.getInfoTypeForString("VICTORY_CULTURAL")]:
			iScore *= 3
			iScore /= 2
		# edead: end
