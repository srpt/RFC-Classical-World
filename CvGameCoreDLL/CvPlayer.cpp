...
PlayerTypes CvPlayer::getSplitEmpirePlayer(int iAreaId) const
{
	...
	*/
	//Rhye - end comment
	//Rhye - start
	int ind1Cities = GET_PLAYER((PlayerTypes)INDEPENDENT1).getNumCities();
	int ind2Cities = GET_PLAYER((PlayerTypes)INDEPENDENT2).getNumCities();

	if (ind1Cities >= ind2Cities) {
		return (PlayerTypes)INDEPENDENT1;
	}
	else {
		return (PlayerTypes)INDEPENDENT2;
	}
	return NO_PLAYER;
	//return ind1Cities >= ind2Cities ? (PlayerTypes)INDEPENDENT1 : (PlayerTypes)INDEPENDENT2;
	//Rhye - end 
}
...
bool CvPlayer::splitEmpire(int iAreaId)
{
	...
	//if (!bPlayerExists)
	if (eNewPlayer >= INDEPENDENT1 && eNewPlayer <= INDEPENDENT4)
	//Rhye - end
	{
		//Rhye - start comment
		/*
