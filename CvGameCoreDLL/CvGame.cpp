...
void CvGame::reset(HandicapTypes eHandicap, bool bConstructorCall)
{
	...
	m_iInitWonders = 0;
	m_iAIAutoPlay = 0;
	m_iCatapultX = 0; // edead
	m_iCatapultY = 0; // edead
...
void CvGame::update()
{
	...
	if (!gDLL->GetWorldBuilderMode() || isInAdvancedStart())
	{
		...
		if (getTurnSlice() == 0)
		{
			...
			if ((getGameTurn() > 0) && !(getGameTurn() < getGameTurnForYear(GET_PLAYER(getActivePlayer()).getStartYear(), getStartYear(), getCalendar(), getGameSpeedType())))
			{
				gDLL->getEngineIFace()->AutoSave(true);
			}
			// edead: end
		...
		if (GET_PLAYER(getActivePlayer()).getStartYear() > GC.getGameINLINE().getStartYear())
		{
			if (getGameTurn() == 0)
				setAIAutoPlay(1);
			else if (getGameTurn() <= getGameTurnForYear(GET_PLAYER(getActivePlayer()).getStartYear(), getStartYear(), getCalendar(), getGameSpeedType()))
...
int CvGame::countVote(const VoteTriggeredData& kData, PlayerVoteTypes eChoice) const
{
	...
	{
		//if (GET_PLAYER((PlayerTypes)iI).isAlive())
		if (GET_PLAYER((PlayerTypes)iI).isAlive() && !GET_PLAYER((PlayerTypes)iI).isBarbarian() && !GET_PLAYER((PlayerTypes)iI).isMinorCiv()) // edead
		{
...
int CvGame::countPossibleVote(VoteTypes eVote, VoteSourceTypes eVoteSource) const
{
	...
	for (iI = 0; iI < MAX_CIV_PLAYERS; iI++)
	{
		if (!GET_PLAYER((PlayerTypes)iI).isBarbarian() && !GET_PLAYER((PlayerTypes)iI).isMinorCiv()) // edead
...
TeamTypes CvGame::findHighestVoteTeam(const VoteTriggeredData& kData) const
{
	...
	if (isTeamVote(kData.kVoteOption.eVote))
	{
		...
		{
			//if (GET_TEAM((TeamTypes)iI).isAlive())
			if (GET_TEAM((TeamTypes)iI).isAlive() && !GET_TEAM((TeamTypes)iI).isBarbarian() && !GET_TEAM((TeamTypes)iI).isMinorCiv()) // edead
			{
...
//Speed: End Modify
		...
		GET_PLAYER(getActivePlayer()).initUnit(((UnitTypes)0), m_iCatapultX, m_iCatapultY); //Rhye (catapult)
	}
}
...
void CvGame::setAIAutoPlayCatapult(int iNewValue)
{
	...
	if (iOldValue != iNewValue)
	{
		...
		if ((iOldValue == 0) && (getAIAutoPlay() > 0))
		{
			CvPlot* pPlot = GC.getMapINLINE().plotINLINE(m_iCatapultX, m_iCatapultY);
			if (pPlot->isUnit()) {
				GC.getMapINLINE().plotINLINE(m_iCatapultX, m_iCatapultY)->getUnitByIndex(0)->kill(false);
				for (int iI = 0; iI < MAX_PLAYERS; iI++)
				{
					if (GET_PLAYER((PlayerTypes)iI).isHuman())
					{
						GC.getMapINLINE().plotINLINE(m_iCatapultX, m_iCatapultY-1)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(m_iCatapultX, m_iCatapultY)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(m_iCatapultX, m_iCatapultY+1)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						//GC.getMapINLINE().plotINLINE(m_iCatapultX+1, m_iCatapultY-1)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						//GC.getMapINLINE().plotINLINE(m_iCatapultX+1, m_iCatapultY)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						//GC.getMapINLINE().plotINLINE(m_iCatapultX+1, m_iCatapultY+1)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(m_iCatapultX-1, m_iCatapultY-1)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(m_iCatapultX-1, m_iCatapultY)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(m_iCatapultX-1, m_iCatapultY+1)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
...
void CvGame::doTurn()
{
	...
	//Rhye - start
	for (iI = 0; iI < MAX_PLAYERS; iI++)
		GET_PLAYER((PlayerTypes)iI).setTurnPlayed(false);
	//Rhye - end
...
/*************************************************************************************************/
	...
	// edead: disable autosave during autoplay
	if (!GC.getDefineINT("DISABLE_AUTOSAVES_DURING_AUTOPLAY") || ((getGameTurn() > 0) && !(getGameTurn() < getGameTurnForYear(GET_PLAYER(getActivePlayer()).getStartYear(), getStartYear(), getCalendar(), getGameSpeedType()))))
	{
...
bool CvGame::testVictory(VictoryTypes eVictory, TeamTypes eTeam, bool* pbEndScore) const
{
		...
		if (GC.getVictoryInfo(eVictory).isEndScore())
		{
			...
			else
			{
				...
				for (int iK = 0; iK < MAX_CIV_TEAMS; iK++)
				{
					...
					if (GET_TEAM((TeamTypes)iK).isAlive() && !GET_TEAM((TeamTypes)iK).isBarbarian() && !GET_TEAM((TeamTypes)iK).isMinorCiv()) // edead
					{
						if (iK != eTeam)
						{
	...
	if (bValid)
	{	
		/* edead: moved to Python as Unique Religious Victory
			...
			if (getNumCivCities() > (countCivPlayersAlive() * 2))
			{
				for (int iK = 0; iK < GC.getNumReligionInfos(); iK++)
				{
					if (GET_TEAM(eTeam).hasHolyCity((ReligionTypes)iK))
					{
						...
						if (iK == RELIGION_SUNNI) iReligionPercent += 10;
						if (iK == RELIGION_ORTHODOXY) iReligionPercent += 5;
						if (iK == RELIGION_CATHOLICISM) iReligionPercent -= 5;
						if (calculateReligionPercent((ReligionTypes)iK) >= iReligionPercent)
	...
	for (int iI = 0; iI < MAX_CIV_TEAMS; iI++)
	{
		...
		if (kLoopTeam.isAlive())
		{
			if (!(kLoopTeam.isMinorCiv()))
			{
				...
				for (int iJ = 0; iJ < GC.getNumVictoryInfos()-2; iJ++) // edead: skip two last conditions (religious & historical)
				//Rhye - end
				{
...
void CvGame::setCatapultXY(int iX, int iY)
{
	m_iCatapultX = iX;
	m_iCatapultY = iY;
}
