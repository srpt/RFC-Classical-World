...
void CvMap::updateCenterUnit()
{
	...
	while (pSelectionNode)
	{
		...
		// edead: start
		// This is a mod-specific hack to avoid using the K-mod pathfinder.
		// The hack assumes max. movement on routes is 1/6, and that there's no paratroopers, 
		// so don't use it for modern scenarios without fixing that!
		
		if (pLoopUnit->getDomainType() == DOMAIN_LAND)
		{
			iRange = std::max(iRange, pLoopUnit->baseMoves() * 6);
		}
		else if (pLoopUnit->getDomainType() == DOMAIN_AIR)
		{
			iRange = std::max(iRange, pLoopUnit->airRange());
		}
		else
		{
			iRange = std::max(iRange, pLoopUnit->baseMoves());
		}
		
		// original K-Mod code below
		/*
		int iLoopRange;
...
void CvMap::calculateAreas()
{
	...
	// By assigning Oman, Bahrain and Mahra separate area IDs, we make the AI treat them as separate
	...
	for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
		...
		if (pLoopPlot->getRegionID() == 31 && !pLoopPlot->isWater())
		{
			pLoopPlot->setArea(bahrainID);
		}
	}
	...
	for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
		...
		if (pLoopPlot->getRegionID() == 32 && !pLoopPlot->isWater())
		{
			pLoopPlot->setArea(mahraID);
		}
	}
