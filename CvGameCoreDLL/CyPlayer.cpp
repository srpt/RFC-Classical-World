...
void  CyPlayer::setStartYear(int iNewValue)
{
	if (m_pPlayer)
		m_pPlayer->setStartYear(iNewValue);
}
...
void  CyPlayer::setGrowthPercent(int iNewValue)
{
	if (m_pPlayer)
		m_pPlayer->setGrowthPercent(iNewValue);
}

void  CyPlayer::setProductionPercent(int iNewValue)
{
	if (m_pPlayer)
		m_pPlayer->setProductionPercent(iNewValue);
}

void  CyPlayer::setResearchPercent(int iNewValue)
{
	if (m_pPlayer)
		m_pPlayer->setResearchPercent(iNewValue);
}

void  CyPlayer::setCulturePercent(int iNewValue)
{
	if (m_pPlayer)
		m_pPlayer->setCulturePercent(iNewValue);
}

void  CyPlayer::setEspionagePercent(int iNewValue)
{
	if (m_pPlayer)
		m_pPlayer->setEspionagePercent(iNewValue);
}

void  CyPlayer::setGreatPeoplePercent(int iNewValue)
{
	if (m_pPlayer)
		m_pPlayer->setGreatPeoplePercent(iNewValue);
}

void  CyPlayer::setReligionSpreadPercent(int /*ReligionTypes*/ eIndex, int iNewValue)
{
	if (m_pPlayer)
		m_pPlayer->setReligionSpreadPercent((ReligionTypes)eIndex, iNewValue);
}

int CyPlayer::AI_getCompactEmpireModifier() const
{
	CvPlayerAI* pPlayer = dynamic_cast<CvPlayerAI*>(m_pPlayer);
	if (pPlayer)
		return pPlayer->AI_getCompactEmpireModifier();
	return 0;
}

void  CyPlayer::AI_setCompactEmpireModifier(int iNewValue)
{
	CvPlayerAI* pPlayer = dynamic_cast<CvPlayerAI*>(m_pPlayer);
	if (pPlayer)
		pPlayer->AI_setCompactEmpireModifier(iNewValue);
}

int CyPlayer::AI_getMassacreProb() const
{
	CvPlayerAI* pPlayer = dynamic_cast<CvPlayerAI*>(m_pPlayer);
	if (pPlayer)
		return pPlayer->AI_getMassacreProb();
	return 0;
}

void  CyPlayer::AI_setMassacreProb(int iNewValue)
{
	CvPlayerAI* pPlayer = dynamic_cast<CvPlayerAI*>(m_pPlayer);
	if (pPlayer)
		pPlayer->AI_setMassacreProb(iNewValue);
}

int CyPlayer::AI_getBuildPersecutorProb() const
{
	CvPlayerAI* pPlayer = dynamic_cast<CvPlayerAI*>(m_pPlayer);
	if (pPlayer)
		return pPlayer->AI_getBuildPersecutorProb();
	return 0;
}

void  CyPlayer::AI_setBuildPersecutorProb(int iNewValue)
{
	CvPlayerAI* pPlayer = dynamic_cast<CvPlayerAI*>(m_pPlayer);
	if (pPlayer)
		pPlayer->AI_setBuildPersecutorProb(iNewValue);
}

int CyPlayer::AI_getWarDistanceModifier() const
{
	CvPlayerAI* pPlayer = dynamic_cast<CvPlayerAI*>(m_pPlayer);
	if (pPlayer)
		return pPlayer->AI_getWarDistanceModifier();
	return 0;
}

void  CyPlayer::AI_setWarDistanceModifier(int iNewValue)
{
	CvPlayerAI* pPlayer = dynamic_cast<CvPlayerAI*>(m_pPlayer);
	if (pPlayer)
		pPlayer->AI_setWarDistanceModifier(iNewValue);
}

int CyPlayer::AI_getWarCoastalModifier() const
{
	CvPlayerAI* pPlayer = dynamic_cast<CvPlayerAI*>(m_pPlayer);
	if (pPlayer)
		return pPlayer->AI_getWarCoastalModifier();
	return 0;
}

void  CyPlayer::AI_setWarCoastalModifier(int iNewValue)
{
	CvPlayerAI* pPlayer = dynamic_cast<CvPlayerAI*>(m_pPlayer);
	if (pPlayer)
		pPlayer->AI_setWarCoastalModifier(iNewValue);
}

int CyPlayer::AI_getMaxTakenTiles() const
{
	CvPlayerAI* pPlayer = dynamic_cast<CvPlayerAI*>(m_pPlayer);
	if (pPlayer)
		return pPlayer->AI_getMaxTakenTiles();
	return 0;
}

void  CyPlayer::AI_setMaxTakenTiles(int iNewValue)
{
	CvPlayerAI* pPlayer = dynamic_cast<CvPlayerAI*>(m_pPlayer);
	if (pPlayer)
		pPlayer->AI_setMaxTakenTiles(iNewValue);
}
...
void CyPlayer::changeDistanceMaintenanceModifier(int iChange)
{
	if (m_pPlayer)
		m_pPlayer->changeDistanceMaintenanceModifier(iChange);
}

void CyPlayer::changeNumCitiesMaintenanceModifier(int iChange)
{
	if (m_pPlayer)
		m_pPlayer->changeNumCitiesMaintenanceModifier(iChange);
}

int CyPlayer::getBorders(int ePlayer) const
{
	return m_pPlayer ? m_pPlayer->getBorders((PlayerTypes)ePlayer) : -1;
}

void CyPlayer::setBorders(int ePlayer, int iNewValue)
{
	if (m_pPlayer)
		m_pPlayer->setBorders((PlayerTypes)ePlayer, iNewValue);
}

int CyPlayer::getAttitudeModifier(int ePlayer) const
{
	return m_pPlayer ? m_pPlayer->getAttitudeModifier((PlayerTypes)ePlayer) : -1;
}

void CyPlayer::setAttitudeModifier(int ePlayer, int iNewValue)
{
	if (m_pPlayer)
		m_pPlayer->setAttitudeModifier((PlayerTypes)ePlayer, iNewValue);
}

void CyPlayer::setFlag(std::wstring s) // Rhye
{
	if (m_pPlayer)
		m_pPlayer->setFlag(s);
}

void CyPlayer::setLeader(int i) // Rhye
{
	if (m_pPlayer)
		m_pPlayer->setLeader(i);
}

int /*LeaderHeadTypes*/ CyPlayer::getLeader() // Rhye
{
